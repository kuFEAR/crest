...
import org.codegist.crest.param.matrixes.common.ISpecialParamsTests;
...
public class SpecialParamsTest extends ISpecialParamsTests<SpecialParamsTest.SpecialParams> {
    ...
    @Path("params/matrix/special")
    public static interface SpecialParams extends ISpecialParamsTests.ISpecialParams {

        @GET
        @Path("inputStream")
        String inputStream(
                @MatrixParam("p1") InputStream p1,
                @MatrixParam("p2") InputStream[] p2);

        @GET
        @Path("reader")
        String reader(
                @MatrixParam("p1") Reader p1,
                @MatrixParam("p2") Reader[] p2);
    }
