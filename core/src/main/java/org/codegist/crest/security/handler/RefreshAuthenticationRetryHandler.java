 ...
 */
public class RefreshAuthenticationRetryHandler implements RetryHandler {

    private final static Logger LOGGER = Logger.getLogger(RefreshAuthenticationRetryHandler.class);
    public static final int DEFAULT_MAX_ATTEMPTS = 1; /* will retry just once in order to refresh the token */

    private final AuthenticationManager authenticationManager;
    private final int max;

    public RefreshAuthenticationRetryHandler(Map<String, Object> customProperties) {
        this.max = Numbers.parse((String) customProperties.get(CRestProperty.HANDLER_RETRY_MAX_ATTEMPTS), DEFAULT_MAX_ATTEMPTS);
        authenticationManager = (AuthenticationManager) customProperties.get(AuthenticationManager.class.getName());
        Validate.notNull(this.authenticationManager, "No authentification manager found, please pass it in the properties (key=" + AuthenticationManager.class.getName() + ")");
    }
